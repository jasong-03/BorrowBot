...
var configSchema = exports_external.object({
  ...
  borrowbotApiUrl: exports_external.string(),
  priceApiUrl: exports_external.string(),
...
`;
function askGeminiRisk(runtime2, positionData) {
  runtime2.log("[Gemini] Querying AI for risk assessment...");
  return queryGemini(runtime2, RISK_SYSTEM_PROMPT, positionData);
}
function askGeminiRebalance(runtime2, positionData) {
  runtime2.log("[Gemini] Querying AI for rebalance strategy...");
  return queryGemini(runtime2, REBALANCE_SYSTEM_PROMPT, positionData);
}
function queryGemini(runtime2, systemPrompt, userPrompt) {
  const geminiApiKey = runtime2.getSecret({ id: "GEMINI_API_KEY" }).result();
  const httpClient = new cre.capabilities.HTTPClient;
  const result = httpClient.sendRequest(runtime2, buildGeminiRequest(systemPrompt, userPrompt, geminiApiKey.value), consensusIdenticalAggregation())(runtime2.config).result();
  runtime2.log(`[Gemini] Response: ${result.geminiResponse}`);
  return result;
}
var buildGeminiRequest = (systemPrompt, userPrompt, apiKey) => (sendRequester, config) => {
  const requestData = {
...
var RISK_REPORT_PARAMS = parseAbiParameters("bytes32, uint8, uint16, uint16, uint16, int16, uint16, bytes32");
var fetchPositionData = (sendRequester, config) => {
  const resp = sendRequester.sendRequest({
    method: "GET",
...
var fetchPriceData = (sendRequester, config) => {
  const resp = sendRequester.sendRequest({
    method: "GET",
...
function onCronTrigger(runtime2, payload) {
  if (!payload.scheduledExecutionTime) {
    throw new Error("Scheduled execution time is required");
  }
  runtime2.log("=== CRE Workflow: BorrowBot Risk Assessment ===");
  runtime2.log(`Triggered at: ${payload.scheduledExecutionTime}`);
  runtime2.log("[Step 1] Fetching position data from BorrowBot API...");
  const httpClient = new cre.capabilities.HTTPClient;
  const position = httpClient.sendRequest(runtime2, fetchPositionData, ConsensusAggregationByFields({
    agentId: median,
  ...
  runtime2.log(`[Step 1] Position: LTV=${position.currentLtvBps}bps, collateral=$${position.collateralValueUsd}, borrow=$${position.borrowValueUsd}`);
  runtime2.log("[Step 2] Fetching market prices...");
  const prices = httpClient.sendRequest(runtime2, fetchPriceData, ConsensusAggregationByFields({
    ethUsd: median,
    btcUsd: median,
    ethChange1h: median,
    ethChange24h: median,
  ...
  runtime2.log(`[Step 2] Prices: ETH=$${prices.ethUsd} (1h: ${prices.ethChange1h}%, 24h: ${prices.ethChange24h}%), BTC=$${prices.btcUsd}`);
  runtime2.log("[Step 3] Querying Gemini AI for risk assessment...");
  ...
  const geminiResult = askGeminiRisk(runtime2, geminiInput);
  const jsonMatch = geminiResult.geminiResponse.match(/\{[\s\S]*"riskLevel"[\s\S]*"confidence"[\s\S]*\}/);
  if (!jsonMatch) {
    throw new Error(`Could not parse Gemini response: ${geminiResult.geminiResponse}`);
  }
  const risk = JSON.parse(jsonMatch[0]);
  if (risk.riskLevel < 0 || risk.riskLevel > 3) {
    throw new Error(`Invalid risk level: ${risk.riskLevel}`);
  }
  if (risk.confidence < 0 || risk.confidence > 1e4) {
    throw new Error(`Invalid confidence: ${risk.confidence}`);
  }
  const riskLabels = ["SAFE", "WARNING", "DANGER", "CRITICAL"];
  runtime2.log(`[Step 3] Risk: ${riskLabels[risk.riskLevel]} (confidence: ${risk.confidence / 100}%), action: ${risk.action}`);
  runtime2.log(`[Step 3] Reasoning: ${risk.reasoning}`);
  runtime2.log("[Step 4] Writing risk assessment to on-chain oracle...");
  ...
  if (writeResult.txStatus !== TxStatus.SUCCESS) {
    throw new Error(`Failed to write risk report: ${writeResult.txStatus}`);
  }
  const txHash = bytesToHex(writeResult.txHash || new Uint8Array(32));
  runtime2.log(`[Step 4] Risk assessment written on-chain: ${txHash}`);
  runtime2.log("=== Risk Assessment Complete ===");
  return txHash;
}
var CRITICAL_RISK_ABI = parseAbi([
  "event CriticalRisk(bytes32 indexed agentId, uint16 currentLtvBps, uint16 maxLtvBps)"
]);
var EXECUTOR_REPORT_PARAMS = parseAbiParameters("bytes32, uint8, uint256, uint16, uint16");
var fetchPositionData2 = (sendRequester, config) => {
  const resp = sendRequester.sendRequest({
    method: "GET",
    url: `${config.borrowbotApiUrl}/v1/cre/position-data`
  }).result();
...
function onCriticalRiskTrigger(runtime2, log) {
  runtime2.log("=== CRE Workflow: Auto-Protect — Critical Risk Detected ===");
  try {
    ...
    const currentLtvBps = decoded.args.currentLtvBps;
    const maxLtvBps = decoded.args.maxLtvBps;
    runtime2.log(`[Step 1] Agent: ${agentId}`);
    runtime2.log(`[Step 1] Current LTV: ${currentLtvBps}bps, Max: ${maxLtvBps}bps`);
    runtime2.log("[Step 2] Fetching current position data...");
    const httpClient = new cre.capabilities.HTTPClient;
    const position = httpClient.sendRequest(runtime2, fetchPositionData2, ConsensusAggregationByFields({
      agentId: median,
      collateralValueUsd: median,
      borrowValueUsd: median,
      vaultBalanceUsd: median,
    ...
    runtime2.log(`[Step 2] Position: collateral=$${position.collateralValueUsd}, borrow=$${position.borrowValueUsd}, vault=$${position.vaultBalanceUsd}`);
    runtime2.log("[Step 3] Querying Gemini for rebalance strategy...");
    const geminiInput = JSON.stringify({
      agentId,
      currentLtvBps: position.currentLtvBps,
      targetLtvBps: position.targetLtvBps,
    ...
    const geminiResult = askGeminiRebalance(runtime2, geminiInput);
    const jsonMatch = geminiResult.geminiResponse.match(/\{[\s\S]*"actionType"[\s\S]*"confidence"[\s\S]*\}/);
    if (!jsonMatch) {
      throw new Error(`Could not parse Gemini rebalance response: ${geminiResult.geminiResponse}`);
    }
    const rebalance = JSON.parse(jsonMatch[0]);
    ...
    runtime2.log(`[Step 3] Strategy: ${actionLabels[rebalance.actionType]}, amount=$${rebalance.amountUsd}, target=${rebalance.targetLtvBps}bps`);
    runtime2.log(`[Step 3] Confidence: ${rebalance.confidence / 100}%`);
    runtime2.log(`[Step 3] Reasoning: ${rebalance.reasoning}`);
    runtime2.log("[Step 4] Writing rebalance command on-chain...");
    const evmConfig = runtime2.config.evms[0];
    const network248 = getNetwork({
      chainFamily: "evm",
      chainSelectorName: evmConfig.chainSelectorName,
    ...
    const txHash = bytesToHex(writeResult.txHash || new Uint8Array(32));
    runtime2.log(`[Step 4] Rebalance command written on-chain: ${txHash}`);
    runtime2.log("=== Auto-Protect Complete ===");
    return txHash;
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    runtime2.log(`[ERROR] Auto-protect failed: ${msg}`);
    throw err;
  }
}
var CRITICAL_RISK_SIGNATURE = "CriticalRisk(bytes32,uint16,uint16)";
